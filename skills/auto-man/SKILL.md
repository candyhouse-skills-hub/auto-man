...
**Name the risk area now, not after delivery.** If the plan touches any of a fixed set of high-blast-radius areas — auth/session/credentials, payment/billing, a data migration or schema change, a public API contract, deleting or skipping existing tests, adding a new external dependency, or anything touching secrets/encryption — say so in the plan itself. This is the same fixed vocabulary `arch-delta.sh` (instantiated in Step 2) mechanically re-derives from the diff at Step 6, and that Step 8's batch review routes on — naming it here isn't extra bookkeeping, it's a cross-check: if Step 6's mechanical pass finds a risk area you didn't name here, that mismatch is itself worth a sentence in the final report.
...
Fill every `{{PLACEHOLDER}}` in `.workflow/verify.sh` (project-specific pass/fail checks — this is the part that can't be templated, it's the actual acceptance logic), `.workflow/goal-condition.txt` (success/safety/no-progress/blocked clauses — all four are mandatory, not just success; only fill `{{JUDGE_CLAUSE}}` if some criterion genuinely needs semantic/visual judgment, see the template's own comments), `.workflow/arch-delta.sh`'s single `{{BASE_REF}}` placeholder (the commit hash you recorded in Step 1 — this one is not a judgment call, just paste the hash), and `CLAUDE.md`'s scope statement. Use the task description the user gave you to derive these; ask a clarifying question if the criteria are genuinely ambiguous rather than guessing.
...
3. Run `.workflow/arch-delta.sh` and append its output as a row to `~/.claude/auto-man/arch-delta.jsonl` (first run on this machine: create it, and copy `${CLAUDE_SKILL_DIR}/templates/arch-delta.schema.json` alongside it), adding `date`, `project`, `runRef` (this run's `LOOP_RUN_ID` or the commit you delivered), `criteriaShapes` (copy it straight from the ledger row you just wrote in step 1 above), and `reviewedAt: null` — **never** write a non-null `reviewedAt` here, only Step 8 may do that, and only after a human has actually looked. This is a separate file from the ledger on purpose: the ledger is quantitative telemetry sized for `MAX_TURNS` derivation, this one is qualitative "what shape changed" sized for the human batch review in Step 8 — don't merge them into one file just because they're both per-run.
   - **If the row's `riskFlags` is non-empty, that changes this run's report below**: say explicitly which flags fired and why (name the files that triggered each), and recommend the user look at this specific diff now rather than waiting for the next Step 8 batch. A risk flag is a mechanical trigger, not a suggestion this session gets to downgrade — don't reason your way out of surfacing one because the change "seems fine."
...
2. Group the pending rows by `project`, then split each group into two buckets:
   - **Needs your eyes now**: any row with a non-empty `riskFlags`, or a `criteriaShapes` that's grep/structural-heavy relative to how the rest of that project's history looks (see Step 2's behavioral-vs-structural guidance) — weak verification is itself a reason to look, independent of whether a risk flag fired.
   - **Delivered clean**: everything else — mechanically verified, no risk flags, no weak-verification signal.
3. For "needs your eyes now," present per row: `project`, `runRef` (so the user can pull the full diff on demand), which `riskFlags` fired and the files that triggered each, `filesTouched`/`diffLoc`. Do not paste the diff itself here — that would just be Step 6's report repeated N times, which is exactly the O(diffs) cost this step exists to avoid.
4. For "delivered clean," present only a rollup per project (e.g. "12 runs, 340 files touched, 0 flags") — a count to skim, not a list to read line by line. That rollup being small and boring is the mechanical layer doing its job, not a gap in this review.
...
6. If review turns up something that should have fired a risk flag but didn't (a real miss in `arch-delta.sh.tmpl`'s heuristics), that's a lesson — route it through the normal Step 6/7 proposal-only channel to widen the risk pattern, don't just patch it silently for this one project.
